...
namespace SJP.Common.EventLogging
{
	...
	/// </summary>
	/// <example> 
	/// Trace.Listeners.Add(new SJPTraceListener(properties,
	///											customPublishers,
	///											errors));
	/// </example>
    public class SJPTraceListener: TraceListener
    {

        #region Events

        // Need declare events as static since subscribers (all of which are static)
		// may register for events before an instance of SJPTraceListener is created.
        ...
        #region Constructors
        /// <summary>
        /// Constructor for SJPTraceListener instances.
        /// </summary>
        ...
        /// <exception cref="SJP.Common.SJPException">Unrecoverable error occured when creating the SJPTraceListener instance.</exception>
        public SJPTraceListener(IPropertyProvider properties,
            IEventPublisher[] customPublishers,
            List<string> errors)
            : base("SJPTraceListener")
        {
            ...
            if (errors.Count == 0)
            {
                ...
                // generated by SJPTraceListener.
                // This is necessary in case they have not yet been created.
                // (Since static classes are created on first use.)
                SJPTraceSwitch sjpTraceSwitch = new SJPTraceSwitch();
                CustomEventSwitch customEventSwitch = new CustomEventSwitch();

                // Notify subscribers that SJPTraceListener has been created successfully
                if (!NotifySubscribers(properties))
                    errors.Add(Messages.SJPTraceListenerSubscriberNotificationFailed);
            }

            if (errors.Count != 0)
            {
                throw new SJPException(String.Format(Messages.SJPTraceListenerConstructor,
                                                    ConfigurationManager.AppSettings["propertyservice.applicationid"],
                                                    ConfigurationManager.AppSettings["propertyservice.groupid"]),
                                                    false, SJPExceptionIdentifier.ELSSJPTraceListenerConstructor);
            }
        }
        #endregion
		...
		private void OnSuperseded(object sender, System.EventArgs s)
		{
			if (SJPTraceSwitch.TraceInfo)
				Trace.Write(new OperationalEvent(SJPEventCategory.Infrastructure,
												 SJPTraceLevel.Info,
												 Messages.SJPTraceListenerPropertyChange));
		...
		/// property provider used to create the SJPTraceListener instance.
		/// </summary>
		/// <remarks>
		...
		/// <exception cref="SJP.Common.SJPException">An error condition prevented the <c>LogEvent</c> from being published by any publisher, including the default publisher.</exception>
		public override void Write(object o)
		{
			...
			{
				throw new SJPException(Messages.SJPTraceListenerUnknownClass, invalidCastException, false, SJPExceptionIdentifier.ELSSJPTraceListenerUnknownObject);
			}
			catch (Exception exception) // catch undocumented exceptions
			{
				throw new SJPException(Messages.SJPTraceListenerUnknownClass, exception, false, SJPExceptionIdentifier.ELSSJPTraceListenerUnknownObject);
			}
				...
				publisher++)
			{
				...
				catch (SJPException sjpException1)
				{
                    ...
                    if ((le is CustomEvent) && (!sjpException1.Logged))
					{
						OperationalEvent error = 
							new OperationalEvent( SJPEventCategory.Infrastructure,
												  SJPTraceLevel.Error,
                                                  String.Format(Messages.ConfiguredPublisherFailed, le.ClassName, eventPublisherMappings[eventArrayIndex][publisher].Target.GetType().FullName, sjpException1.Message),
                                                  sjpException1);
						...
						if (le is CustomEvent)
						{
							OperationalEvent warning = 
								new OperationalEvent(SJPEventCategory.Infrastructure,
													 ...
													 String.Format(Messages.SJPTraceListenerDefaultPublisherUsed, le.ClassName));

							Trace.Write(warning);
						}

					}
                    catch (SJPException sjpException2)
					{
						// All publishers failed to log! This exception will filter up to the SJP trace listener client.
                        throw new SJPException(Messages.SJPTraceListenerDefaultPublisherFailure, sjpException2, false, SJPExceptionIdentifier.ELSDefaultPublisherFailed);
					}
				}
		...
		/// <summary>
		/// This prototype is not supported by SJPTraceListener.
		/// An OperationalEvent is logged to highlight that it has been called.
		/// </summary>
		public override void Write(string s)
		{
			Write(new OperationalEvent(SJPEventCategory.Infrastructure,
									   SJPTraceLevel.Warning,
									   String.Format(Messages.SJPTraceListenerUnsupportedPrototype, s)));
		}
		...
		/// <summary>
		/// This prototype is not fully supported by SJPTraceListener.
		/// An OperationalEvent is logged to highlight that it has been called.
		/// </summary>
        ...
        private bool NotifySubscribers(IPropertyProvider properties)
        {
            ...
            if (validator.ValidateProperty(Keys.OperationalTraceLevel, errors))
            {
                ...
                SJPTraceLevel operationalTraceLevelValue
                    = (SJPTraceLevel)PropertyValidator.StringToEnum(typeof(SJPTraceLevel), operationalTraceLevel);
            ...
            IEventPublisher[] customPublishers)
        {
            ...
            int numOfEvents = customEventIds.GetLength(0) +
                typeof(SJPTraceLevel).GetFields().GetLength(0) - 2; // values for Undefined and Off are not used for events
            ...
            AssignEventToPublishers(properties,
                ...
                eventArrayIndexer,
                OperationalEvent.ClassNamePrefix + SJPTraceLevel.Error.ToString(),
            ...
            AssignEventToPublishers(properties,
                ...
                eventArrayIndexer,
                OperationalEvent.ClassNamePrefix + SJPTraceLevel.Warning.ToString(),
                standardPublishers, customPublishers);
            ...
            AssignEventToPublishers(properties,
                ...
                eventArrayIndexer,
                OperationalEvent.ClassNamePrefix + SJPTraceLevel.Verbose.ToString(),
                standardPublishers, customPublishers);
            ...
            AssignEventToPublishers(properties,
                ...
                eventArrayIndexer,
                OperationalEvent.ClassNamePrefix + SJPTraceLevel.Info.ToString(),
                standardPublishers, customPublishers);
